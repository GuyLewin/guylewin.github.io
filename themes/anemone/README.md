...
> **Vendored copy.** This was a git submodule tracking
> [Speyll/anemone](https://github.com/Speyll/anemone) at `dd1650c`. Upstream has been
> inactive since mid-2025 and never migrated to Zola 0.23 (Tera v2), which broke the
> build, so the theme was vendored here and patched locally:
>
> - `atom.xml`: dropped `| escape` — Tera v2 auto-escapes XML, so the explicit filter was
>   double-escaping feed content (a bug that predates the migration).
> - `section.html`: `paginator.pages` → `paginator?.pages`; Tera v2 errors on undefined
>   variables instead of treating them as falsy.
> - `base.html`: same optional chaining for `page`, plus the `head` block that
>   `page.html` / `blog/page.html` already tried to override but no parent defined.
> - `templates/shortcodes/`: removed — Zola 0.23 dropped shortcodes entirely.
> - Demo-site files (`content/`, `config.toml`, `screenshot.png`) removed.
>
> Upstream is no longer pulled in. Edit these files directly.
